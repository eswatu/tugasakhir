...
async function paginate(model, pageIndex, pageSize, sortColumn = 'id', sortOrder = "ASC" , filterColumn, filterQuery)
{
...
async function pageuser(model, pageIndex, pageSize, sortColumn = 'id', sortOrder = "ASC" , filterColumn, filterQuery, uid)
{
    const page = parseInt(pageIndex) || 0;
    const take = parseInt(pageSize) || 10;
    const skip = page  * take;
    ...
    if (filterQuery && filterColumn) {
        if (filterQuery.length > 0 && filterQuery != undefined || filterQuery != "" && filterColumn != "") {
            if (typeof(parseInt(filterQuery)) == "number") {
                filters = { filterColumn: parseInt(filterQuery) };
            } else if(filterQuery === 'true' || filterQuery === 'false') {
                const want = (filterQuery === 'true');
                filters = {filterColumn: want};
            } else {
            filters = { filterColumn: filterQuery };
            }
        }
    }
    let myOrder;
